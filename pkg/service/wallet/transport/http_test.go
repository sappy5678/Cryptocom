...
import (
	...
	"github.com/sappy5678/cryptocom/pkg/service/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID cryptocom.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr cryptocom.User) (cryptocom.User, error) {
					usr.ID = 1
			...
			rbac: &mock.RBAC{
				UserFn: func(c echo.Context) cryptocom.AuthUser {
					return cryptocom.AuthUser{
						...
						CompanyID:  2,
						LocationID: 3,
						Role:       cryptocom.SuperAdminRole,
						Email:      "john@mail.com",
					}
				}},
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *cryptocom.ListQuery, p cryptocom.Pagination) ([]cryptocom.User, error) {
					...
					return nil, cryptocom.ErrGeneric
				},
			},
			wantStatus: http.StatusOK,
			wantResp: &listResponse{
				Users: []cryptocom.User{
					{
						Base: cryptocom.Base{
							ID:        10,
							CreatedAt: mock.TestTime(2001),
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (cryptocom.User, error) {
					return cryptocom.User{
						Role: &cryptocom.Role{
							AccessLevel: cryptocom.CompanyAdminRole,
						},
					}, nil
				},
			},
		...
		},
		{
			...
			udb: &mockdb.User{
				...
				DeleteFn: func(orm.DB, cryptocom.User) error {
					return nil
				},
			},
			rbac: &mock.RBAC{
